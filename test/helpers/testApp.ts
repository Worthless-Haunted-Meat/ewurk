...
import type { UserStore, TokenStore, SessionStore, Mailer, OutboxEntry, AuthServicePort } from '../../src/ports/auth.js';
...
/**
 * An AuthServicePort backed by the fake user/token/session/mail stores,
 * re-implementing the exact algorithm documented in
 * src/services/authService.ts's doc comment (sha256 token hash, 15-minute
 * link expiry, 12-hour session expiry). This lets EVERY OTHER bounded
 * context's route-level tests (donations/devices/leases/families/classes)
 * sign in and exercise real routes through the real
 * src/http/middleware/session.ts without depending on AuthService's own
 * adapters being implemented yet — AuthService's own behavior is covered
 * directly in test/auth.test.ts.
 */
export class FakeAuthService implements AuthServicePort {
  constructor(
    private users: FakeUserStore,
    private tokens: FakeTokenStore,
    private sessions: FakeSessionStore,
    private mailer: FakeMailer,
  ...
  verifyMagicLink(rawToken: string): { sessionId: string; user: User } {
    ...
    const expiresAt = new Date(this.clock.now().getTime() + 12 * 60 * 60_000).toISOString();
  ...
  getSessionUser(sessionId: string): User | null {
    ...
    if (new Date(session.expiresAt).getTime() < this.clock.now().getTime()) return null;
...
export function buildFakeDeps(): FakeBag {
  ...
  // Likewise: every OTHER bounded context's route tests sign in through
  // sessionMiddleware, which calls deps.authService.getSessionUser on
  // every request. Using the real (still-a-stub-until-T1) AuthService
  // here would 500 every authenticated request in every other task's
  // route tests. FakeAuthService is a working reference implementation of
  // the same contract; AuthService's own behavior is covered directly in
  // test/auth.test.ts.
  const authService = new FakeAuthService(users, tokens, sessions, mailer, clock);
