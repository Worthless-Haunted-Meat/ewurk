...
describe('R25/R21: platform routes (wired app, fakes)', () => {
  ...
  test('[R21] staff can reach /donations (web, 200) — the role gate lets them through', async () => {
    // Uses /donations, not /classes: GET /donations reads straight off the
    // DonationStore port (deps.donations.list()) with no service call, so
    // this exercises only the role gate, not any bounded context's own
    // (independently red-until-implemented) service logic.
